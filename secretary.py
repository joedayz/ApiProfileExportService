...
Patched to work on python3 by Daniel Berg <mail@roosta.sh>
    - Lint file using flake8
...
class UndefinedSilently(Undefined):
    ...
    return_new = lambda *args, **kwargs: UndefinedSilently()  # noqa: E731
...
class Renderer(object):
    ...
    def finalize_value(self, eval_ctx, value):
        ...
        * if field is a control flow ({% %}), then we find immediate node of
        ...
        for tag in self._tags_in_document(document):
            ...
            if scale_to.startswith(('after::', 'before::')):
                # Don't remove whole field tag, only "text:text-input"
                # container
    ...
    def add_media_to_archive(self, media, mime, name=''):
        ...
        node = self.create_node(self.manifest, 'manifest:file-entry',
                                files_node)
    ...
    def _render_xml(self, xml_document, **kwargs):
        ...
        try:
            ...
            final_xml = parseString(result.encode(
                'ascii',
                'xmlcharrefreplace'
            ))
    ...
    def render(self, template, **kwargs):
        ...
        self.content.getElementsByTagName('office:document-content')[0] \
            .replaceChild(
        ...
        self.files['content.xml'] = self.content.toxml().encode(
            'ascii',
            'xmlcharrefreplace'
        )
        self.files['styles.xml'] = self.styles.toxml().encode(
            'ascii',
            'xmlcharrefreplace'
        )
        self.files['META-INF/manifest.xml'] = self.manifest.toxml().encode(
            'ascii',
            'xmlcharrefreplace'
        )
    ...
    def insert_style_in_content(self, style_name,
                                attributes=None,
                                **style_properties):
        """
        Insert a new style into content.xml's <office:automatic-styles>
    ...
    def markdown_filter(self, markdown_text):
        ...
        try:
            ...
            raise SecretaryError(
                'Could not import markdown2 library. \
                Install it using "pip install markdown2"'
            )
        ...
        for tag in transform_map:
            ...
            for html_node in html_nodes:
                odt_node = xml_object.createElement(
                    transform_map[tag]['replace_with']
                )
                ...
                if html_node.hasChildNodes():
                    ...
                    # a container like text:p. When there's not a double
                    # linebreak separating list elements, markdown2 creates
                    # <li> elements without wraping their contents inside a
                    # container. Here we automatically create the container if
                    # one was not created by markdown2.
                    if (tag == 'li' and
                            html_node.childNodes[0].localName != 'p'):
                        ...
                        def traverse_preformated(node):
                            if node.hasChildNodes():
                                ...
                                container = xml_object.createElement(
                                    'text:span'
                                )
                    ...
                    if name not in styles_cache:
                        ...
                        if style_node is None:
                            ...
                            style_node = self.insert_style_in_content(
                                name, transform_map[tag]['style'].get(
                                    'attributes',
                                    None
                                ),
        ...
        ODTText = ''.join(node_as_str for node_as_str in map(
            node_to_string,
